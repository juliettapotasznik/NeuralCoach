...
# Add the pose_extractor directory to Python path
pose_extractor_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'pose_extractor')
sys.path.append(pose_extractor_path)
...
def parse_poses(inference_results, input_scale, stride, fx, is_video=False):
    ...
    for pose_id in range(poses_3d.shape[0]):
        ...
        if is_video and pose_id < len(current_poses_2d):
        ...
        if is_video and pose_id < len(current_poses_2d):
            pose_ids.append(current_poses_2d[pose_id].id)
        else:
            pose_ids.append(pose_id)

    return np.array(translated_poses_3d), np.array(poses_2d_scaled), np.array(pose_ids, dtype=np.int32)
